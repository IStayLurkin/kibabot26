...
class ChatCommands(commands.Cog):
    ...
    async def handle_chat_turn(self, destination, author, channel, content: str):
        ...
        logger.info("[chat] user=%s msg=%r", str(author), content[:500])
            ...
            try:
                ...
                if intent in ("draw", "sing"):
                    ...
                    if reply.content:
                        ...
                        logger.info("[chat] bot=%r", reply.content[:500])
                        content = reply.content
                        if len(content) <= 2000:
                            await destination.send(content)
                        else:
                            await send_long_message(destination, content)
    ...
    @commands.is_owner()
    async def forget_all(self, ctx, target: discord.Member | None = None):
        """[Owner only] Wipes all memory (chat history, summary, key-value, and vector/RAG) for a user. Usage: !forgetall @user"""
        from database.chat_memory import delete_user_history
        from database.vector_memory_db import delete_vector_memories
        from database.db_connection import get_db
        if target is None:
            await ctx.send("Usage: `!forgetall @user`")
            return
        ...
        embed = discord.Embed(
            ...
            description=f"All memory (chat history, summary, and RAG) for {target.mention} has been deleted.",
            color=discord.Color.dark_red(),
        )
        await ctx.send(embed=embed)
